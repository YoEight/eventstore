...
readAllEvents Settings{..} c_pos p_pos max_c tos dir = Operation create
  where
    ...
    pending :: forall a. Word8 -> Input 'Pending (Slice 'All)
    pending cmd (Arrived Package{..})
        | packageCmd == cmd =
            Right $ decodeResp packageData $ \resp ->
                let r      = getField $ _Result resp
                    err    = getField $ _Error resp
                    c_pos  = getField $ _CommitPosition resp
                    p_pos  = getField $ _PreparePosition resp
                    ...
                    slice  = AllSlice f_pos n_pos eos evts dir in
